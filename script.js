...
const App = (() => {
  ...
  function filtered() {
    ...
    return Vocabulary.filter(entry => entry.memberships.some(item => (!state.source || item.source === state.source) && (!$('#category').value || item.topic === $('#category').value))
  ...
  function dictionary() {
    ...
    $('#count').textContent = `Найдено: ${entries.length.toLocaleString('ru')}`;
  ...
  function cards() {
    ...
    actions.append(button('Ещё учу', 'secondary', () => { setKnown(entry, false); state.card++; cards(); }), button('✓ Знаю', 'primary', () => { setKnown(entry, true); state.card++; cards(); }));
  ...
  function startQuiz() {
    ...
    const uniqueWords = [...new Map(shuffle(entries).map(entry => [entry.word.toLocaleLowerCase('en'), entry])).values()];
    const translations = [...new Set(entries.map(item => item.translation))];
    const meanings = new Map();
    for (const item of Vocabulary) {
      const key = item.word.toLocaleLowerCase('en');
      if (!meanings.has(key)) meanings.set(key, new Set());
      for (const meaning of item.translation.split(' / ')) meanings.get(key).add(meaning.toLocaleLowerCase('ru'));
    }
    const questions = [];
    // Формируем только 10 вопросов, чтобы не нагружать телефон всей коллекцией.
    for (const entry of uniqueWords) {
      const accepted = meanings.get(entry.word.toLocaleLowerCase('en'));
      const distractors = shuffle(translations.filter(value => !value.split(' / ').some(meaning => accepted.has(meaning.toLocaleLowerCase('ru'))))).slice(0, 3);
  ...
  function quiz() {
    ...
    if (session.index >= session.questions.length) {
      ...
      box.append(button('Пройти ещё раз', 'primary', startQuiz)); study.append(box); container.append(study); return;
    }
    ...
    for (const option of question.options) {
      const answerButton = button(option, 'answer', () => {
        ...
        feedback.textContent = correct ? 'Верно! Так держать.' : `Правильный ответ: ${question.entry.translation}`;
        next.hidden = false;
        Storage.write('lastQuiz', { correct: session.answers.filter(answer => answer.correct).length, answered: session.answers.length, total: session.questions.length });
        next.focus();
      });
  ...
  function navigate(view) {
    state.view = ['dictionary', 'rules', 'cards', 'quiz'].includes(view) ? view : 'dictionary';
    document.body.dataset.view = state.view;
    for (const id of ['dictionary', 'rules', 'cards', 'quiz']) $(`#${id}`).hidden = id !== state.view;
    ...
    const labels = { dictionary: ['ВАША КОЛЛЕКЦИЯ', 'Словарь'], rules: ['ОТ ПРАВИЛА К ПРАКТИКЕ', 'Правила и примеры'], cards: ['ВСПОМНИТЬ И ЗАПОМНИТЬ', 'Карточки'], quiz: ['ПРОВЕРЬТЕ СЕБЯ', 'Небольшой тест'] };
    ...
    else { if (!state.quiz) startQuiz(); else quiz(); }
  }
  function init() {
    ...
    function filterChanged() { state.limit = 40; state.deck = []; state.quiz = null; navigate(state.view); }
